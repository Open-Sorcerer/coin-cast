...
const NavItem = ({ link, icon, name }: NavItemProps) => {
  return (
    <li className="w-[80%] text-gray-600 dark:text-gray-300 font-semibold flex pb-2">
      <Link href={link} className="flex justify-center sm:justify-start items-center py-2">
        {icon}
    ...
    <aside className="fixed min-h-[100vh] w-[45] md:w-[180px] border-r border-gray-400 py-5">
      <div className="px-3 justify-start items-center">
        <Link href="/" className="flex justify-start items-center mt-2 mb-8">
          <Image src="/coincast.png" width="30" height="30" alt="coincast" className="mr-3" />
          <span className="hidden md:flex font-bold text-xl dark:text-[#F8FBFC]">Coincast</span>
        </Link>
        <ul className="flex flex-col items-center justify-center w-full">
          {navSec1.map((item) => (
            <NavItem key={item.link} link={item.link} icon={item.icon} name={item.name} />
          ))}
